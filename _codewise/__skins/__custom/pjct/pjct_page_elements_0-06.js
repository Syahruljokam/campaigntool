...
Revision History:
  0.01	- Initial version. TB, yyyy-mm-dd, ??:??
...
// WebFontConfig = { google: { families: [ 'Ubuntu', 'Pathway Gothic One' ] } };
// CSI_appendScript('src=' + ('https:' == URLProtocol ? 'https' : 'http') + ':\/\/ajax.googleapis.com\/ajax\/libs\/webfont\/1\/webfont.js', 'type=text\/javascript', 'async=async'); // Note that this will fail locally for IE9, due to a "@font-face failed cross-origin request" error


/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Internal style sheet */

// iSSHTML += ' @font-face { font-family: "Open Sans SemiBold"; src: url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_700.eot?iefix) format("embedded-opentype"), url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_700.woff) format("woff"); }';
// iSSHTML += ' @font-face { font-family: "Open Sans"; src: url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_400.eot?iefix) format("embedded-opentype"), url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_400.woff) format("woff"); }';
/* if (is_ie && (!is_ie9up) && (URLProtocol == 'file:')) {
	iSSHTML += ' @font-face { font-family: "Open Sans SemiBold"; src: url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_700.eot); }'; // format("embedded-opentype")
	iSSHTML += ' @font-face { font-family: "Open Sans"; src: url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One_400.eot); }'; // format("embedded-opentype")
...
if (is_ie && (!is_ie9up)) {
	if (URLProtocol == 'file:') {
		...
		iSSHTML += ' body, acronym, button, blockquote, input, li, ol, option, p, select, textarea, th, td, ul { font-family: "Pathway Gothic One", "Courier New", sans-serif; }';
		...
		iSSHTML += ' body, acronym, button, blockquote, input, li, ol, option, p, select, textarea, th, td, ul { font-family: "Pathway Gothic One", sans-serif; }';
		iSSHTML += ' h1, h2, h3, h4, h5, h6 { font-family: "Pathway Gothic One", sans-serif; }';
	};
} else {
	iSSHTML += ' @font-face { font-family: "Pathway Gothic One"; src: url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One\/Pathway_Gothic_One_400.eot?iefix) format("embedded-opentype"), url(' + fDPO + mFPO + 'media\/fonts\/Pathway_Gothic_One\/Pathway_Gothic_One_400.woff) format("woff"); }';
};

iSSHTML += ' span.icon, div.ExpandOMatic span.ExpandOMaticGraphic, a.tooltipIcon, div.sliderNavPanelButtons div.sliderNavPanelButtonsInner button, #search form input.form-submit, li.primaryNav.menuParent a.primaryNav, span.sf-sub-indicator, div.logoCarouselNavPrev div.logoCarouselNavPrevIcon, div.logoCarouselNavNext div.logoCarouselNavNextIcon { background-image: url(' + fDPO + mFPO + 'media\/images\/icons\/_icon_sprite_01.png); }';
...
iSSHTML += ' .highslide-controls, .highslide-controls ul, .highslide-controls a { background-image: url(' + fDPO + mFPO + '_highslide\/graphics\/controlbar-bluish.gif) !important; }';

iSSHTML += ' #takeActionHeader { background-image: url(' + fDPO + mFPO + 'media\/images\/logos\/ndp_logo_02_trans.png); }';
iSSHTML += ' #headerContainerOuter, #primaryNavContainer, #bannerContainerOuter, #bannerContainer #bannerTabs div.bannerTabLinkContainer a, #footerContainerOuter { background-image: url(' + fDPO + mFPO + 'media\/images\/_gui_sprite_01.jpg); }';
...
iSSHTML += ' #bannerRightColumn { background-image: url(' + fDPO + mFPO + 'media\/images\/_pj_headshot_01.png); }';
...
if (is_ie) {
	...
	if (is_ie9up) {
		if (is_ie9) {
			...
			if (URLProtocol == 'file:') {
				iSSHTML += ' @font-face { font-family: "Ubuntu"; src: url(' + fDPO + mFPO + 'media\/fonts\/Ubuntu.eot?iefix) format("embedded-opentype"); }';
				...
				// iSSHTML += ' @font-face { font-family: "Open Sans"; src: url(' + fDPO + mFPO + 'media\/fonts\/Open_Sans.eot?iefix) format("embedded-opentype"); }';
			};
		};
	...
	// IE9 and lower
	if (!is_ie10up) {
		iSSHTML += ' .greyGradient { -ms-filter: "progid:DXImageTransform.Microsoft.gradient(StartColorStr=\'#ffffff\', EndColorStr=\'#e0e0e0\')"; }'; /* IE9 & IE8 MUST have double quotes here */
		...
		// IE7 and lower
			...
			iSSHTML += ' .greyGradient { filter: progid:DXImageTransform.Microsoft.gradient(StartColorStr=\'#ffffffff\', EndColorStr=\'#ffe0e0e0\'); }'; /* IE7 CAN'T have double quotes here, and needs "ff" for the alpha value */
...
if (is_ipod || is_iphone || is_ipad || is_safari_mobile) {
	...
	if (is_ipad) {
		iSSHTML += ' #mobileMenuContainer { display: none !important; }';
	};
};
...
function aPL() { // animatePageLoad
	if ((document.getElementById) && (!pM) && (pageType != 'blank') && (!is_wince)) {
		// setTimeout('cEV(' + "'logo','show'" + '); cEO(' + "'logo',100" + ');', pADIT);
	};
};
...
// Restyle transparent PNG images
function PJCT_rTPNGI($_currentImage) {
	...
	if (is_ie && !is_ie7up) {
		...
		if ($_currentImage) {
			$_currentImage
				.css({
					'filter': 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src=' + $_currentImage.attr('src') + ')' // Need to test this!
				})
				.attr('src', blank_transparent_image.src);

		// Otherwise run through ALL of the transparentPNG classed images in the document...
		} else {
			$('img').each( function(loopCounter) {
				$_currentImage = $_allImageTags.eq(loopCounter);

				if ($_currentImage.attr('usemap') != '') {
					$_currentImage.css({
						'border': 'none'
					});
				};

				if ($_currentImage.hasClass('transparentPNG')) {
					imgWidth = $_currentImage.attr('width');
					imgHeight = $_currentImage.attr('height');
					imgSrc = $_currentImage.attr('src');

					$_currentImage
						.attr('width', imgWidth)
						.attr('height', imgHeight)
						.css({
							'filter': 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true, src=' + imgSrc + ')'
						})
						.attr('src', blank_transparent_image.src);
				};
			});
...
function PJCT_sHBTTL() {
	setTimeout(function() {
		if ($(document).height() > $(window).height()) {
			$('li#backToTopLinkContainer').animate({
				'width': 35,
				'padding-left': 10
			}); //.show()
		} else {
	...
	if (document.getElementsByTagName) {
		...
		// if ($_head) {
			highslideScript = document.createElement('script');
				highslideScript.src = fDPO + mFPO + '_highslide\/highslide.js';
				highslideScript.type = 'text\/javascript';

			highslideStyles = document.createElement('link');
				highslideStyles.href = fDPO + mFPO + '_highslide\/highslide.css';
				highslideStyles.type = 'text\/css';
				highslideStyles.rel = 'stylesheet';

			$_head.appendChild(highslideStyles);
			$_head.appendChild(highslideScript);
		// };
	};
	*/
...
function PJCT_pI_sLM(menuDataLoadSuccess) { // Stop trying to load the JSON menu data
	...
	if (menuDataLoadSuccess) {
		...
		if (isRemotePage || menuLoaderForced) {
			MenuReworker.reworkMenu('footerNav01');

			MenuReworker.reworkMenu('footerNav02');

			MenuReworker.reworkMenu('footerNav03');

			MenuReworker.reworkMenu('footerNav04');
		};
...
function PJCT_pI_hN() { // highlightNavigation
	...
	$('a.primaryNav, a.secondaryNav, a.thirdLevelNav, a.fourthLevelNav, a.utilityNav, a.footerNav, a.footerUtilityNav').each( function(loopCounter_linksToHighlight) {
		...
		if ($(this).hasClass('footerUtilityNav')) {
			if ($(this).text().toLowerCase() == 'site credits') {
...
function PJCT_pI_rH() {
	if ($_header.exists()) {
		...
		// Mobile menu
		if ($_mobileMenuContainer.exists()) { // && isMobileVersion
			$_headerContainerOuter.prepend($_mobileMenuContainer);
		};

		// Logo
		if ($_logoContainer.exists() && $_logo.exists()) {
			if (isMobileVersion) {
		...
		if ($_logoContainer.exists()) { // && $_logo.exists()
			...
			if (skinSwitcherEnabled) {
				...
				$_logoLink = $_logo.closest('a');

				if ($_logoLink.exists()) { // && !isMobileVersion
					// Create an image map for the logo
					logoLinkImageMapHTML = '<map id="logoImageMap" name="logoImageMap">';
						logoLinkImageMapHTML += '<area href="' + liveServerName + '" shape="poly" coords="30,12,146,12,146,36,134,78,38,72,30,42" title=" Go back to our home page... " \/>';
						// logoLinkImageMapHTML += '<area href="' + liveServerName + '" shape="circle" coords="50,50,40" title=" Go back to our home page... " \/>';
						// logoLinkImageMapHTML += '<area href="' + liveServerName + '" shape="rect" coords="1,1,99,10" title=" Go back to our home page... " \/>';
					logoLinkImageMapHTML += '<\/map>';
					$_logoLink.append($(logoLinkImageMapHTML));

					// Because IE sucks
					if (is_ie) {
						if ($_logoLink.attr('href') == 'about:\/') {
							$_logoLink.attr('href', liveServerName);
						};
						$('> map', $_logoLink).css({
							'cursor': 'pointer'
						});
					...
					if (!isHomePage && ($_logoLink.attr('href') != URLMinusQueryString) && ((liveServerName + $_logoLink.attr('href')) != URLMinusQueryString) && (URLMinusQueryString != liveServerName)) {
						$_logo
							...
							.attr('alt', '')
							.attr('title', '')
							.attr(usemap, '#logoImageMap');
					};

					// Either way, disable the logo link
					$_logoLink
						.removeAttr('href')
						.bind('click', function() {
							// return false;
		...
		if ($_utilityNav.exists()) {
			...
			$('a', $_utilityNav).each( function() {
				// Take action link
				if ($(this).text().toLowerCase() == 'take action') {
					$_takeActionLink = $(this);

					$_takeActionLinkIcon = $('<span class="icon takeActionIcon" \/>');
						$_takeActionLinkIcon.hover(function () {
							$('a.takeActionLink', $_utilityNav).addClass('active');
						}, function() {
							$('a.takeActionLink', $_utilityNav).removeClass('active');
						}).bind('click', function() {
					...
					$_takeActionLink
						.hover(function () {
							$_takeActionLinkIcon.addClass('active');
						}, function() {
							$_takeActionLinkIcon.removeClass('active');
						})
		...
		if ($_primaryNav.exists()) {
			...
			$_primaryNavContainer.append($_primaryNav);
		...
		if ($_takeActionForm.exists()) {
			...
			$('input[type="text"]', $_takeActionForm).each( function(loopCounter_takeActionFormFields) {
				$(this)
					...
					.bind('focus', function() {
						...
						if (($(this).val() == takeActionFormWarningText[loopCounter_takeActionFormFields]) || ($(this).val() == $(this).attr('initialValue'))) {
							$(this).val('');
						};
					})
					.bind('blur', function() {
			...
			if ($_takeActionFormSubmitButton.exists()) {
				$_takeActionFormSubmitButton.bind('click', function() {
					...
					// Name
					if (($_takeActionFormNameField.val() == $_takeActionFormNameField.attr('initialValue')) || ($_takeActionFormNameField.val() == takeActionFormNameFieldWarningText) || ($_takeActionFormNameField.val().length <= 4) || ($_takeActionFormNameField.val().indexOf(' ') == -1)) {
						$_takeActionFormNameField.addClass('warning');

						$_takeActionFormNameField
							.val(takeActionFormNameFieldWarningText)
							.css({
								'color': '#ef2f24',
					...
					if (!emailAddressRegExp.test($_takeActionFormEmailField.val())) {
						...
						$_takeActionFormEmailField
							.val(takeActionFormEmailFieldWarningText)
							.css({
								'color': '#ef2f24',
								'background-color': '#ffa3a3',
								'border-color': '#ef2f24'
					...
					// Postal code
					// if (($_takeActionFormPostalField.val() == $_takeActionFormPostalField.attr('initialValue')) || ($_takeActionFormPostalField.val() == takeActionFormPostalFieldWarningText) || (($_takeActionFormPostalField.val().length < 6) || (($_takeActionFormPostalField.val().length == 7) && ($_takeActionFormPostalField.val().indexOf(' ') == -1)))) {
					if (!postalCodeRegExp.test($_takeActionFormPostalField.val())) {
						$_takeActionFormPostalField.addClass('warning');

						$_takeActionFormPostalField
							.val(takeActionFormPostalFieldWarningText)
							.css({
								'color': '#ef2f24',
								'background-color': '#ffa3a3',
								'border-color': '#ef2f24'
	...
	function PJCT_showTakeActionContent() {
		...
		$_takeActionLinkIcon.closest('li').toggleClass('active');
			...
			if ($_searchTextField.exists()) {
				...
				$_searchTextField
					...
					.bind('focus', function() {
						...
						if (($(this).val() == searchFieldWarningText) || ($(this).val() == $(this).attr('initialValue'))) {
							$(this).val('');
						};
					})
					.bind('blur', function() {
				...
				$_searchSubmitButton
					.bind('click', function() {
						if ($_searchForm.exists()) {
							if (($_searchTextField.val() == '') || ($_searchTextField.val() == ' ') || ($_searchTextField.val() == searchFieldWarningText) || ($_searchTextField.val() == $_searchTextField.attr('initialValue'))) {
								$_searchTextField
									.addClass('warning')
									.val(searchFieldWarningText)
									.css({
										'color': '#f5862d'
									}).delay(1000).animate({
										'color': '#999999'
...
function PJCT_pI_rC() {
	...
	if ($_content.exists()) {
		...
		if (isMobileVersion) {
			...
			$('#content h1:first').appendTo($_contentContainerInner);
		};
	...
	function PJCT_pI_rC_restyleSliders() {
		...
		$('div.slidesOuter').each( function(loopCounter_sliderContainers) {
			...
			$_slides.each( function() {
				var lastLink = $(' a:last', $(this));
				if (lastLink.exists()) { // ($(this).hasClass('slideExtractLink')) && 
					if (!(lastLink).attr('name')) {
						var thisLinkClass = lastLink.attr('class');
						if (thisLinkClass) {
						...
						$(this).css({ // .removeClass('leftAligned rightAligned')
							'cursor': 'pointer'
						}).bind('click', function() {
							if (!($(' a:last', $(this)).hasClass('fake'))) {
								if (is_ie && !is_ie8up) {
						...
						}).wrap('<a class="' + $(this).attr('linkClass') + '" href="' + $(' a:last', $(this)).attr('href') + '"><\/a>');
					};
				};
			});
			...
			if (sliderLength[loopCounter_sliderContainers] > 1) {
				...
				sliderNavDirectionButtonsTopHTML += '<div id="sliderNav' + (loopCounter_sliderContainers + 1) + 'PrevTop" class="sliderNavPrev"><img id="sliderNav' + (loopCounter_sliderContainers + 1) + 'PrevIconTop" class="sliderNavPrevIcon sliderNavPrevIconTop" src="' + blank_transparent_image.src + '" title=" &lt; Previous " alt="" \/><\/div>';
				sliderNavDirectionButtonsTopHTML += '<div id="sliderNav' + (loopCounter_sliderContainers + 1) + 'NextTop" class="sliderNavNext"><img id="sliderNav' + (loopCounter_sliderContainers + 1) + 'NextIconTop" class="sliderNavNextIcon sliderNavNextIconTop" src="' + blank_transparent_image.src + '" title=" Next &gt; " alt="" \/><\/div>';
				...
				sliderNavDirectionButtonsHTML += '<div id="sliderNav' + (loopCounter_sliderContainers + 1) + 'Prev" class="sliderNavPrev"><img id="sliderNav' + (loopCounter_sliderContainers + 1) + 'PrevIcon" class="sliderNavPrevIcon" src="' + blank_transparent_image.src + '" title=" &lt; Previous " alt="" \/><\/div>';
				sliderNavDirectionButtonsHTML += sliderNavStepButtonsHTML;
				sliderNavDirectionButtonsHTML += '<div id="sliderNav' + (loopCounter_sliderContainers + 1) + 'Next" class="sliderNavNext"><img id="sliderNav' + (loopCounter_sliderContainers + 1) + 'NextIcon" class="sliderNavNextIcon" src="' + blank_transparent_image.src + '" title=" Next &gt; " alt="" \/><\/div>';
				...
				// $(this).after($(sliderNavDirectionButtonsHTML));
			};
		});

		// Social media links, if "bannerContainer" exists
		if ($_bannerContainerOuter.exists()) {
	...
	function PJCT_pI_rC_restyleSliders2() {
		...
		// Slider nav top
		$('img.sliderNavPrevIconTop').each( function() {
			$(this).bind('click', function() {
				$('img.sliderNavPrevIcon', $(this).closest('div.sliderNavDirectionButtons').parent()).not('.sliderNavPrevIconTop').trigger('click');
			});
		});
		$('img.sliderNavNextIconTop').each( function() {
			$(this).bind('click', function() {
				$('img.sliderNavNextIcon', $(this).closest('div.sliderNavDirectionButtons').parent()).not('.sliderNavNextIconTop').trigger('click');
			});
		});
		...
		$_movieContainers.each( function(loopCounter) {
			...
			$_movieContainerOuterContainers[loopCounter] = $('<div id="movieContainerOuterContainer' + (loopCounter + 1) + '" class="movieContainerOuterContainer" \/>');
				...
				if ($(this).parents('div.highslideVideo').exists()) {
					if (movieFileWidths[loopCounter] > 640) {
						$(this).parents('div.highslideVideo').addClass('highslide-html-content-ultra-wide');
			...
			if ((((movieF4Vs[loopCounter] != '') || (movieFLVs[loopCounter] != '')) && (flashinstalled == 2) && (flashversion >= FlashPlayerVersionRequired)) || (((movieMOVs[loopCounter] != '') || (movieMP4s[loopCounter] != '')) && QuickTimePlayerInstalled)) {
				...
				// QuickTime version
					...
					$_movieWrappersForegroundImages[loopCounter]
						...
						.attr('alt', ' Click to start playing this video... ')
						.attr('title', ' Click to start playing this video... ')
						.css({
			...
			if ($(this).parents('div.highslideVideo').exists()) {
				...
				// Otherwise, use some default link text
					...
					$_movieHighslideLinks[loopCounter]
						.text('Click to open video...')
						.attr('title', 'Click to open video...');
				};
	...
	function PJCT_pI_rC_iHIG() {
		$('div.highslideImageGallery').each(function() {
			hs.addSlideshow({
		...
		$('a.highslideHTML').livequery( function() {
			this.onclick = function() {
				...
				if (is_firefox) {
					...
					return hs.htmlExpand(this, { contentId: this.getAttribute('popupId'), width: thisPopupWidth }); //, height: $('#' + this.getAttribute('popupId')).height() //This causes a highslide htmlPopup with subcolumns to incorrectly height size
	...
	function PJCT_pI_rC_rASC() {
		...
		$('div.styled').not($('#rightColumn div.styled')).each( function() {
			/* styledContainerId = '';
			if ($(this).attr('id') == 'rightColumn') {
				styledContainerId = ' id="fullWidthPageRightColumnContainer"'
				// $(this).parent('id', 'fullWidthPageRightColumnContainer');
			};
		...
		// User login for
		$('#user-login-form ul li a').each( function() {
			$(this).addClass('styled');
		});
	};
	...
	function PJCT_pI_rC_restyleTeaserContent() {
		// if (isMobileVersion) {
			$('div.viewDate').each( function() {
				...
				$('div.viewDate.styledOrange').closest('div.viewDateWrapper')
//					.addClass('styledOrange');

				$('div.viewDate.styledGreen').closest('div.viewDateWrapper')
//					.addClass('styledGreen');
	...
	function PJCT_pI_rC_rCLC() {
		if ($_contentLower.exists() && $_logoCarousel.exists()) {
			...
			if (isMobileVersion) {
				var logoCarouselText = 'swipe with 2 fingers to scroll';
				if (is_android) {
					logoCarouselText = 'swipe with one finger to scroll';
				};
				$_contentLower.prepend('<div class="logoCarouselText">' + logoCarouselText + '<\/div>');
			} else {
				$_logoCarouselNav.html('<div class="logoCarouselNavPrev greyGradientInverted"><div class="logoCarouselNavInner greyGradient"><div class="logoCarouselNavPrevIcon"><\/div><\/div><\/div><div class="logoCarouselNavNext greyGradientInverted activated"><div class="logoCarouselNavInner greyGradient"><div class="logoCarouselNavNextIcon activated"><\/div><\/div><\/div>');
				$_logoCarouselNav.insertAfter($_logoCarousel);
			};

			// Activate the logo carousel nav?
			if (logoCarouselLength > 5) {
...
function PJCT_pI_rCBT() {
	...
	if (is_iphone || is_ipod || is_safari_mobile) {
		if (isMobileVersion) {
...
function rFP() {
	...
	$_header = $('#header');
		...
		$_search = $('#search');
			...
			// $_searchTextField = $('#edit-search-block-form-1');
			$_searchTextField = $('#edit-search-block-form--2');
		...
		$_takeAction = $('#takeAction');
			$_takeActionForm = $('#takeActionForm');
				$_takeActionFormNameField = $('input[name="name"]', $_takeActionForm);
				$_takeActionFormEmailField = $('input[name="email"]', $_takeActionForm);
				$_takeActionFormPostalField = $('input[name="postal"]', $_takeActionForm);
	...
	$_headerContainerOuter = $('<div id="headerContainerOuter" \/>');
		...
		$_headerContainerInner = $('<div id="headerContainerInner" \/>');
			...
			// $_headerBannerContainer = $('<div id="headerBannerContainer" \/>');
	...
	// Embed movies
	PJCT_pI_eM();
	// setTimeout('PJCT_pI_eM();', 500);

	// Restyle all "transparentPNG" -classed images
	PJCT_rTPNGI();
