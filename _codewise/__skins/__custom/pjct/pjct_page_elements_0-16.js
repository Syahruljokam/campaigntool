...
if (is_webkit) {
	...
	if (is_safari) {
		...
		if (!is_ipad) {
			iSSHTML += ' #bannerContainer #bannerTabs div.bannerTabLinkContainer + div.bannerTabLinkContainer { margin-top: 2px; }';
		};
	};
};
...
function PJCT_pI_iLQ() {
	...
	$('a.tooltipIcon, a.tooltipLink').livequery( function() {
		...
		$(this)
			...
			.dynamic({
				// top: { offset: [(is_ipad * window.pageYOffset), -150] }
			})
/*			onBeforeShow : function(a, b){
      // fix for ipad
      if(isiPad)  {
         selectedObj.data('tooltip').getConf().offset = [$(window).scrollTop(), 0];
      }
    },
    ; */
	});
