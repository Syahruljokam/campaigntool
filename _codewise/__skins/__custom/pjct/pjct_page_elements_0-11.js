			...
			$_slides.each( function() {
				...
				if ($_firstLink.exists() && !isMobileVersion) {
					if (!($_firstLink).attr('name')) {
						...
						}).wrap('<a class="' + $(this).attr('linkClass') + '" href="' + $_firstLink.attr('href') + '"><\/a>');
					};
					/*
					$(this).css({ 
						'cursor': 'pointer'
	...
	$_header = $('#header');
		...
		$_takeAction = $('#takeAction');
			...
			$_takeActionForm = $('#takeActionForm');
				$_takeActionFormFirstNameField = $('input[name="firstName"]', $_takeActionForm);
				$_takeActionFormLastNameField = $('input[name="lastName"]', $_takeActionForm);
