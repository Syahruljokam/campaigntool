...
iSSHTML += ' span.icon, div.ExpandOMatic span.ExpandOMaticGraphic, a.tooltipIcon, div.sliderNavPanelButtons div.sliderNavPanelButtonsInner button, #search form input.form-submit, li.primaryNav.menuParent a.primaryNav, span.sf-sub-indicator { background-image: url(' + fDPO + mFPO + 'media\/images\/icons\/_icon_sprite_01.png); }';
...
function PJCT_pI_rF() {
	if ($_footer.exists()) {
		// Footer logo
		if (!isMobileVersion) {
			if ($_footerLogoContainer.exists() && $_footerLogo.exists()) {
				// $_footerLogo).addClass('transparentPNG');
				$_footerContainerInner.append($_footerLogoContainer);
			};
		};

		// Footer navigation
		if ($_footerNav.exists()) {
			$('div:first', $_footerNav).addClass('first'); // For IE6's benefit
			$_footerContainerInner.append($_footerNav);
		};
...
function rFP() {
	...
	$_header = $('#header');
		...
		$_search = $('#search');
			...
			// $_searchSubmitButton = $('#search-button');
	...
	// Footer elements
	$_footer = $('#footer');
		$_footerNav = $('#footerNav');
		$_footerLogoContainer = $('#footerLogoContainer');
			$_footerLogo = $('#footerLogo');
		$_copyright = $('#copyright');
		$_footerUtilityNav = $('#footerUtilityNav');
		$_FlashWarning = $('#FlashWarning');
		$_JavaScriptWarning = $('#JavaScriptWarning');
