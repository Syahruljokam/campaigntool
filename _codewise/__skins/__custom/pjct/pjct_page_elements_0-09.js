...
if (languagePreference == 'fr') {
	...
	var ui_clickToPlayVideo = ui_clickToPlayVideo_fr;
	...
	if (!is_ie10up) {
		...
		if (!is_ie9up) {
			...
			if (!is_ie8up) {
				...
				iSSHTML += ' form#search-block-form input.form-submit { height: 28px; margin-top: -31px; }';
	...
	function PJCT_pI_rC_restyleSliders() {
		...
		$('div.slidesOuter').each( function(loopCounter_sliderContainers) {
			...
			$_slides.each( function() {
				var lastLink = $(' a:last', $(this) && !isMobileVersion);
				if (lastLink.exists()  & !isMobileVersion) {
						...
						$(this).css({ // .removeClass('leftAligned rightAligned')
							...
							if (!($(' a:last', $(this)).hasClass('fake'))) {
								if (is_ie && !is_ie8up) {
									...
									if ($(' a:last', $(this)).attr('class').indexOf('highslide') != -1) {
										return false;
									};
								};
							};
	...
	function PJCT_resizeHomePageSubColumnHeights() {
		...
		if ($('div.subColumnContainer div.subColumn3 div.latestNews').exists()) {
			...
			$('div.subColumn3 div.styled', $_homePageSubColumnContainer).animate({
				'height': (homePageSubColumnMaxHeight - 0) // Why the fudge factor?
			}, 1000);

			$('div.subColumn3 div.tab', $_homePageSubColumnContainer).animate({
				'min-height': (homePageSubColumnMaxHeight + 4 - 47) // Account for the fudge factor, plus the height of the tabs
			}, 1000);
		};
	};
...
function rFP() {
	...
	if (isMobileVersion && (URL.indexOf('_test_page_v2') == -1)) {
		// setTimeout( function() {
